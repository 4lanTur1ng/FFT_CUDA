...
int gemm(const int M,
    ...
    warmup = 2;
    for (int run = 0; run < iterations / 2 + warmup; run++)//这里iterations/2是为了平衡cpu和gpu的计算负载
    {
        float duration = cpu_kernel(A, B, C_host, M, N, K);
        if (run >= warmup) duration_cpu += duration;
    }
    duration_cpu = duration_cpu / (iterations / 2);//源代码错误,忘记加括号
...
int main() {
    ...
    int errCode = gemm(n, n, n, /* GEMM size, M, N, K */
        4,             /* workgroup size */
        10,            /* repeat time */
        my_gpu_queue);
